...
function PrincipleSection({ title, statement, revealText, sectionIdx }: PrincipleProps) {
  ...
  return (
    <div
      ref={containerRef}
      className="flex flex-col lg:flex-row gap-8 lg:gap-16 items-start py-12 border-b border-secondary-container/50 last:border-b-0"
    >
      {/* Left Column: Bebas Neue Title */}
      <div className="w-full lg:w-4/12">
        <h2
          className="font-normal text-6xl sm:text-7xl lg:text-[90px] text-on-background uppercase tracking-tight leading-none"
      ...
      <div className="w-full lg:w-8/12 flex flex-col justify-center">
        ...
        <p className="text-xl sm:text-2xl lg:text-3xl font-headline-display font-extrabold uppercase text-on-background tracking-tight leading-snug mb-6">
          {statement}
        </p>
        ...
        <p
          ref={revealRef}
          className="text-lg sm:text-xl lg:text-2xl font-headline-display font-extrabold uppercase tracking-tight leading-snug"
        >
          {words.map((word, i) => (
            <span
              key={i}
...
export default function AnimatedPrinciples() {
  ...
  return (
    <section className="w-full max-w-[1250px] mx-auto bg-surface-container-lowest p-8 md:p-14 rounded-2xl border border-secondary-container shadow-sm space-y-4">
      {/* Header */}
